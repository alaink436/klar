/**
 * App Store + Play Store badges as inline SVG.
 * Both render greyed-out + non-clickable when href is undefined.
 *
 * Designed to match Apple's "Download on the App Store" and Google's
 * "Get it on Google Play" official treatment, but rebuilt as minimal
 * SVG so we control monochrome variant + greyed state.
 */
...
export function AppleBadge({ href, className = "" }: Props) {
  ...
  if (!href) {
    return (
      <span
        className={`${badgeBase} ${badgeDisabled} ${className}`}
        aria-disabled="true"
      >
        {inner}
      </span>
