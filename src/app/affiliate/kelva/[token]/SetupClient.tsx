...
// Kelva thin wrapper around the shared OnboardingShell. Posts the completed
// payout form to /api/affiliate/complete which proxies into the Kelva
...
export function SetupClient({ token, handle, displayName, language }: { token: string; handle: string; displayName: string; language?: string }) {
  ...
  return <OnboardingShell brand="kelva" handle={`@${handle}`} onSubmit={onSubmit} lang={lang} />;
}
