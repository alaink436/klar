...
// Promillo thin wrapper around the shared OnboardingShell. Posts the
// completed payout form to /api/affiliate/complete which proxies into the
// Promillio Supabase via service-role and triggers agreement-log +
// confirmation email.
...
export function SetupClient({ token, handle, displayName, language }: { token: string; handle: string; displayName: string; language?: string }) {
  ...
  return <OnboardingShell brand="promillio" handle={`@${handle}`} onSubmit={onSubmit} lang={lang} />;
}
