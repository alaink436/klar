...
// KLAR_INBOX_URL/KEY, the Inquiry interface and SOURCE_META/SOURCE_KEYS/
// sourceLabel/sourcePill moved to /admin/inbox/page.tsx with the inbox view.
...
// Reply-composer assets (REPLY_TEMPLATE_JSON, replyTemplateSelectOptions,
// REPLY_INBOX_JS) moved to ./_reply.ts — shared by outreach (here) + the
// migrated /admin/inbox route.
...
// inboxView + Inquiry/isTestInquiry/SOURCE_*/sourcePill migrated to /admin/inbox/page.tsx (reply-composer in ./_reply).
...
export async function GET(req: Request): Promise<Response> {
  ...
  if (view === "outreach") {
    ...
    // Inbox migrated to its own React route. Forward filter/msg params so
    // POST-handler redirects (approve/decline) and old bookmarks keep working.
    return redirectTo(url, "/admin/inbox");
  }
